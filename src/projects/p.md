---
layout: layouts/project.njk
title: Prototype
slug: Prototype
tag: Visual Identity
year: 2026
description: "Prototype is a speculative record label for experimental sound studies, built as a system-based visual identity.
              Instead of focusing on individual releases, the project explores how a structured framework can organize, connect, and continuously expand a body of work."
permalink: "/projects/{{ slug }}/"

media:
  primaryVideo: "/assets/projects/p/intro.mp4"
  secondaryImage: "/assets/projects/p/post_01.jpg"
  secondaryAlt: "Prototype preview image"

detail:
  startingPoint: "Prototype is conceived as a speculative record label for experimental sound studies. While  

    Individual releases are not treated as discrete outputs, but as positions within a larger structure, allowing the label to operate as an archive rather than a brand. This structure is articulated through a simple indexing logic (P//001, P//002, …), which organizes the work as a continuous sequence rather than a collection of separate entries.

    As the system expands, the archive remains in flux: each new release reconfigures the context of the existing ones, producing meaning through relation, accumulation, and temporal progression."
  outcome: "The result is a modular visual system that differentiates between the overall structure and individual releases. Color acts as the primary layer of orientation:
    purple represents the system, while yellow marks individual sound studies.
    New releases visually exist between these two states, becoming part of the system as they are introduced.

    This logic is consistently applied across all touchpoints. Each application follows the same rules, allowing the identity to remain coherent while adapting to different formats."
  categories:
    - Logo
    - Type selection
    - Color system
    - Social Media concept

gallery:
  - layout: full
    type: video
    src: "/assets/projects/p/intro.mp4"
    alt: "Full video 01" 

  - layout: double
    items:
      - type: video
        src: "/assets/projects/p/animation.mp4"
        label: "Double video 01"
      - type: image
        src: "/assets/projects/p/color.jpg"
        alt: "Double image 01"

  - layout: full
    type: video
    src: "/assets/projects/p/web_animation.mp4"
    alt: "Full video 02" 

  - layout: double
    items:
      - type: image
        src: "/assets/projects/p/insta_feed.jpg"
        label: "Double image 02"
      - type: video
        src: "/assets/projects/p/phone_animation.mp4"
        alt: "Double video 02"

      

---